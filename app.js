...
// 2. DATA STORAGE LAYER (Cloudflare Workers KV Backend Sync with Bearer Auth)
const Storage = {
  save: async function() {
    const token = localStorage.getItem("pionotes_token");
    if (!token) return;
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        console.error("Sunucuya kaydetme başarısız:", data.error || "Bilinmeyen hata");
      }
    } catch (e) {
  ...
  load: async function(callback) {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        localStorage.removeItem("pionotes_token");
        localStorage.removeItem("pionotes_username");
        localStorage.removeItem("pionotes_role");
      ...
      if (res.ok && data) {
        ...
        if (state.settings.caglaUnlocked === undefined) state.settings.caglaUnlocked = false;
      } else {
        console.warn("Kullanıcı verisi alınamadı, varsayılan yüklendi.");
        state = defaultState;
      }
      if (callback) callback();
    } catch (e) {
      console.error("Veri yükleme hatası, varsayılan yükleniyor:", e);
      state = defaultState;
      if (callback) callback();
    }
  }
...
// 4. THEME CONTROLLER
function updateSecretThemeElements() {
  try {
    // Malatyalı
    const malatyaUnlocked = state.settings && state.settings.malatyaUnlocked;
    ...
    // Çağla
    const caglaUnlocked = state.settings && state.settings.caglaUnlocked;
    const isCaglaThemeActive = state.settings && state.settings.theme === "cagla";
    
    const logoFlower = document.getElementById("logo-flower");
    const headerFlower = document.getElementById("header-flower");
    const caglaThemeBtn = document.getElementById("theme-cagla-btn");
    
    if (logoFlower) {
      if (isCaglaThemeActive) {
        logoFlower.classList.remove("hidden");
      } else {
        logoFlower.classList.add("hidden");
      }
    }
    
    if (headerFlower) {
      if (isCaglaThemeActive) {
        headerFlower.classList.remove("hidden");
      } else {
        headerFlower.classList.add("hidden");
      }
    }
    
    if (caglaThemeBtn) {
      if (caglaUnlocked) {
        caglaThemeBtn.classList.remove("hidden");
      } else {
        caglaThemeBtn.classList.add("hidden");
      }
    }

    // Custom Accent Selector Dots visibility matching theme unlocks
    const accentCaglaDot = document.getElementById("accent-cagla-dot");
    const accentMalatyaDot = document.getElementById("accent-malatya-dot");
    
    if (accentCaglaDot) {
      if (caglaUnlocked) {
        accentCaglaDot.classList.remove("hidden");
      } else {
        accentCaglaDot.classList.add("hidden");
      }
    }
    
    if (accentMalatyaDot) {
      if (malatyaUnlocked) {
        accentMalatyaDot.classList.remove("hidden");
      } else {
        accentMalatyaDot.classList.add("hidden");
      }
    }
  } catch (e) {
    console.error("updateSecretThemeElements error:", e);
  }
}
...
function initTheme() {
  try {
    ...
    // Always sync secret elements visibility first
    updateSecretThemeElements();
    
    // Remove all theme classes first
    DOM.body.classList.remove("dark-theme", "light-theme", "gradient-theme", "cyberpunk-theme", "matrix-theme", "sunset-theme", "malatya-theme", "cagla-theme");
...
function toggleTheme() {
  ...
  if (currentTheme === "dark") {
    ...
    if (state.settings && state.settings.caglaUnlocked) {
      nextTheme = "cagla";
    } else if (state.settings && state.settings.malatyaUnlocked) {
      nextTheme = "malatya";
    } else {
      nextTheme = "dark";
    }
  } else if (currentTheme === "cagla") {
  ...
  if (nextTheme === "malatya") {
    applyAccentColor("malatya");
  } else if (nextTheme === "cagla") {
    applyAccentColor("cagla");
  } else {
  ...
  const themeNames = {
    ...
    cagla: "Prenses Çağla Teması 🌸👑 Etkinleştirildi!",
...
const ACCENT_COLORS = {
  ...
  cagla: {
    dark: { main: "#db2777", hover: "#be185d" },
    light: { main: "#ec4899", hover: "#db2777" }
  },
  malatya: {
    dark: { main: "#f97316", hover: "#ea580c" },
    light: { main: "#ea580c", hover: "#c2410c" }
  }
};
...
function createNoteCardDOM(note) {
  ...
  card.addEventListener("dragstart", (e) => {
    e.stopPropagation();
    e.dataTransfer.setData("text/type", "note");
    e.dataTransfer.setData("text/id", note.id);
  ...
  card.addEventListener("dragend", (e) => {
    ...
    document.querySelectorAll(".note-card").forEach(el => {
      el.classList.remove("dragover-left", "dragover-right");
    });
  });
  ...
  card.addEventListener("dragover", (e) => {
    ...
    if (relX < rect.width / 2) {
      card.classList.add("dragover-left");
      card.classList.remove("dragover-right");
    } else {
      card.classList.add("dragover-right");
      card.classList.remove("dragover-left");
    }
  });

  card.addEventListener("dragleave", (e) => {
    card.classList.remove("dragover-left", "dragover-right");
  });
  ...
  card.addEventListener("drop", (e) => {
    ...
    if (type !== "note" || draggedId === note.id) return;

    const rect = card.getBoundingClientRect();
    const relX = e.clientX - rect.left;
    const position = relX < rect.width / 2 ? "before" : "after";
    reorderNotes(draggedId, note.id, position);
  });
  ...
  card.addEventListener("click", (e) => {
    ...
    // Secret trigger for presentation (blank note with title "not90220")
    const isSecret = (note.title && note.title.trim() === "not90220");
    const isCaglaSecret = (note.title && (note.title.trim().toLowerCase() === "çağla" || note.title.trim().toLowerCase() === "cagla"));
    const isEmpty = !note.content || note.content.replace(/<[^>]*>/g, '').trim() === "";
...
function renderFolderGroup(title, emoji, folderNotes, isFirst, folderId) {
  const groupContainer = document.createElement("div");
  groupContainer.className = "folder-group-container";
  groupContainer.dataset.folderId = folderId;
  groupContainer.style.marginBottom = "24px";

  const header = document.createElement("div");
  header.className = "folder-group-title";
  header.style.marginTop = isFirst ? "0px" : "24px";

  // Drag and drop for folders in All Notes section
  header.draggable = true;
  header.addEventListener("dragstart", (e) => {
    e.dataTransfer.setData("text/type", "folder");
    e.dataTransfer.setData("text/id", folderId);
    groupContainer.classList.add("dragging-folder");
  });

  header.addEventListener("dragend", (e) => {
    groupContainer.classList.remove("dragging-folder");
    document.querySelectorAll(".folder-group-container").forEach(el => {
  ...
  groupContainer.addEventListener("dragover", (e) => {
    ...
    if (relY < rect.height / 2) {
      groupContainer.classList.add("dragover-above");
      groupContainer.classList.remove("dragover-below");
    } else {
      groupContainer.classList.add("dragover-below");
      groupContainer.classList.remove("dragover-above");
    }
  });

  groupContainer.addEventListener("dragleave", () => {
    groupContainer.classList.remove("dragover-above", "dragover-below");
  });
  ...
  groupContainer.addEventListener("drop", (e) => {
    ...
    if (type === "folder") {
      ...
      reorderFolders(draggedId, folderId, position);
    } else if (type === "note") {
      moveNoteToFolder(draggedId, folderId);
    }
  });

  header.innerHTML = '<span class="folder-group-emoji">' + emoji + '</span><span class="folder-group-name">' + title + '</span><span class="folder-group-badge">' + folderNotes.length + ' Not</span>';
  ...
  DOM.notesContainer.appendChild(groupContainer);
}
...
function renderNotes() {
  ...
  // Sort notes: array order will be preserved since we no longer sort by createdAt
  ...
  if (filtered.length === 0) {
    ...
    if (folderFilter === "all") {
      DOM.notesContainer.classList.remove("notes-grid");
      
      let isFirst = true;
      
      // Render special folders
      state.folders.forEach(folder => {
        const folderNotes = filtered.filter(n => n.folderId === folder.id);
        if (folderNotes.length > 0) {
          renderFolderGroup(folder.name, folder.emoji, folderNotes, isFirst, folder.id);
          isFirst = false;
        }
      });
      
      // Render uncategorized notes
      const uncategorizedNotes = filtered.filter(n => n.folderId === "none");
      if (uncategorizedNotes.length > 0) {
        renderFolderGroup("Klasörsüz Notlar", "📥", uncategorizedNotes, isFirst, "none");
      }
    } else {
      DOM.notesContainer.classList.add("notes-grid");
      // Draw note cards directly
...
function deleteFolder(folderId) {
  if (confirm("Bu klasörü silmek istediğinize emin misiniz? Klasörün içindeki notlar SİLİNMEYECEK, sadece 'Klasörsüz' yapılacaktır.")) {
    ...
    // Set matching notes to none folderId
    state.notes = state.notes.map(note => {
      if (note.folderId === folderId) {
        return { ...note, folderId: "none" };
      }
      return note;
    });
...
function openEditNoteModal(noteId) {
  try {
    ...
    if (isSecret && isEmpty) {
      ...
      return; // Do not open the edit modal for the secret note
    }
...
function saveNote() {
  ...
  // Validate on plain text inside editor (bypass for secret blank note)
  if (!plainText && !content) {
    const isSecretTitle = (title === "not90220" || title.toLowerCase() === "çağla" || title.toLowerCase() === "cagla");
    if (isSecretTitle) {
      // Allow saving blank note for the secret launch trigger
    } else {
  ...
  if (editingNoteId) {
    ...
    state.notes = state.notes.map(note => {
      if (note.id === editingNoteId) {
        return {
          ...note,
          title: title || (plainText.substring(0, 15) + (plainText.length > 15 ? "..." : "")),
          content,
          folderId,
          color,
          createdAt: Date.now()
...
function saveFolder() {
  try {
    console.log("saveFolder triggered");
    ...
    showToast("Klasör kaydedilirken hata: " + err.message, "error");
  }
}
...
function setupEventListeners() {
  ...
  if (DOM.saveSettingsBtn) DOM.saveSettingsBtn.addEventListener("click", closeSettingsModalFunc);
    ...
    DOM.themePickerContainer.addEventListener("click", (e) => {
      ...
      if (btn) {
        ...
        if (selectedTheme === "malatya") {
          applyAccentColor("malatya"); // Auto set to malatya orange
        } else if (selectedTheme === "cagla") {
          applyAccentColor("cagla"); // Auto set to cagla rose/pink
        } else {
...
function startMalatyaShow() {
  try {
    ...
    initTheme();
    applyAccentColor("malatya"); // Force beautiful custom orange accent
...
function reorderFolders(draggedId, targetId, position) {
  const fromIndex = state.folders.findIndex(f => f.id === draggedId);
  const toIndex = state.folders.findIndex(f => f.id === targetId);
  if (fromIndex === -1 || toIndex === -1 || fromIndex === toIndex) return;

  const [draggedFolder] = state.folders.splice(fromIndex, 1);
...
function reorderNotes(draggedId, targetId, position) {
  const fromIndex = state.notes.findIndex(n => n.id === draggedId);
  const toIndex = state.notes.findIndex(n => n.id === targetId);
  if (fromIndex === -1 || toIndex === -1 || fromIndex === toIndex) return;

  const draggedNote = state.notes[fromIndex];
  const targetNote = state.notes[toIndex];
  ...
  let newIndex = state.notes.findIndex(n => n.id === targetId);
  if (position === "after") {
    newIndex += 1;
  }
  
  state.notes.splice(newIndex, 0, draggedNote);
...
function moveNoteToFolder(noteId, folderId) {
  ...
  let insertIndex = state.notes.length;
  for (let i = state.notes.length - 1; i >= 0; i--) {
    if (state.notes[i].folderId === folderId) {
      insertIndex = i + 1;
      break;
    }
  }
  
  state.notes.splice(insertIndex, 0, note);
...
// 13. INITIALIZATION & SESSION MANAGEMENT
  ...
  loginForm.onsubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        localStorage.setItem("pionotes_token", data.token);
        localStorage.setItem("pionotes_username", data.username);
        localStorage.setItem("pionotes_role", data.role);
        ...
        // Update user badge
        const prettyRole = data.role === "admin" ? "Yönetici" : "Kullanıcı";
        document.getElementById("logged-user-badge").innerText = `Oturum: ${data.username} (${prettyRole})`;
        if (data.role === "admin") {
        ...
        // Load user state & init dashboard
        Storage.load(() => {
          initTheme();
          applyAccentColor(state.settings.accentColor || "indigo");
          setupEventListeners();
          ...
          renderFolders();
          renderNotes();
          initFrontendAuthAndAdmin();
        });
      } else {
        loginError.innerText = data.error || "Giriş yapılamadı.";
        loginError.classList.remove("hidden");
      }
    } catch (err) {
      console.error(err);
      loginError.innerText = "Sunucu ile bağlantı kurulamadı.";
      loginError.classList.remove("hidden");
    } finally {
...
function initFrontendAuthAndAdmin() {
  ...
  if (closeAdminModal) closeAdminModal.onclick = () => adminModal.classList.add("hidden");
  if (adminDoneBtn) adminDoneBtn.onclick = () => adminModal.classList.add("hidden");
  
  async function loadAdminUsersList() {
    const token = localStorage.getItem("pionotes_token");
    if (!token) return;
    
    adminUsersList.innerHTML = "<tr><td colspan='3' style='text-align:center;'>Yükleniyor...</td></tr>";
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        data.users.forEach(user => {
          const dateStr = user.createdAt ? new Date(user.createdAt).toLocaleString("tr-TR") : "-";
          ...
          delBtn.onclick = async () => {
            if (confirm(`${user.username} adlı kullanıcıyı ve tüm verilerini silmek istediğinize emin misiniz?`)) {
              try {
                ...
                if (delRes.ok && delData.success) {
                  showToast(`${user.username} başarıyla silindi.`);
                  loadAdminUsersList();
                } else {
                  showToast(delData.error || "Silme işlemi başarısız.", "error");
                }
              } catch (e) {
        ...
        adminUsersList.innerHTML = `<tr><td colspan='3' style='text-align:center; color:var(--note-red);'>Hata: ${data.error || "Veriler alınamadı."}</td></tr>`;
      }
    } catch (e) {
      adminUsersList.innerHTML = "<tr><td colspan='3' style='text-align:center; color:var(--note-red);'>Sunucu bağlantı hatası.</td></tr>";
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (token) {
    ...
    const username = localStorage.getItem("pionotes_username") || "Kullanıcı";
    const role = localStorage.getItem("pionotes_role") || "user";
    const prettyRole = role === "admin" ? "Yönetici" : "Kullanıcı";
    document.getElementById("logged-user-badge").innerText = `Oturum: ${username} (${prettyRole})`;
...
function createFallingApricot() {
  try {
    ...
    console.error("Apricot error:", e);
  }
}

function startCaglaShow() {
  try {
    // 1. Screen Shake
    DOM.body.classList.add("screen-shake");
    setTimeout(() => {
    ...
    // 2. Create Overlay
    const overlay = document.createElement("div");
    overlay.className = "cagla-overlay";
    
    const title = document.createElement("div");
    title.innerText = "Ya sen çiço musuuun??? ❤️";
    ...
    // 4. Unlock Çağla Theme
    let justUnlocked = false;
    if (!state.settings.caglaUnlocked) {
      state.settings.caglaUnlocked = true;
      justUnlocked = true;
    }
    
    state.settings.theme = "cagla";
    Storage.save();
    
    // Initialize theme & elements
    initTheme();
    applyAccentColor("cagla"); // Force beautiful custom rose pink accent
    
    if (justUnlocked) {
      showToast("Özel Prenses 'Çağla' Teması Açıldı! 🌸👑");
    } else {
      showToast("ÇAĞLA PRENSES GÜCÜ AKTİFLEŞTİRİLDİ! 🌸✨");
    }
  } catch (e) {
    console.error("Cagla show failed:", e);
  }
}
...
function createFallingFlower() {
  try {
    ...
    // Random position and animation variables
    const startX = Math.random() * window.innerWidth;
    const duration = 2.5 + Math.random() * 2.5; // 2.5 to 5.0 seconds
    const delay = Math.random() * 0.3; // 0 to 300ms
    const size = 1.0 + Math.random() * 1.5; // random scale
    const drift = -100 + Math.random() * 200; // horizontal movement drift
    
    flower.style.left = `${startX}px`;
    flower.style.setProperty("--fall-duration", `${duration}s`);
    flower.style.setProperty("--fall-delay", `${delay}s`);
    flower.style.setProperty("--drift-x", `${drift}px`);
    ...
    console.error("Flower rain error:", e);
  }
}
